...
import { tileAlign } from '../../utils/dungeon';
...
export function generateBossArena(floor: number, seed: number): DungeonData {
  return bossArenaGenerator.generate(floor, seed) as DungeonData;
}

export const bossArenaGenerator: TerrainGenerator = {
  type: 'boss_arena',
  generate(floor: number, seed: number): TerrainData {
    ...
    const itemSpawns = [
      { id: `boss_item_1`, x: tileAlign(hallX - TILE_SIZE), y: tileAlign(hallY + hallH / 2 - TILE_SIZE), type: 'health' as const },
      { id: `boss_item_2`, x: tileAlign(hallX - TILE_SIZE), y: tileAlign(hallY + hallH / 2 + TILE_SIZE), type: 'energy' as const },
    ];
...
registerTerrain(bossArenaGenerator);
