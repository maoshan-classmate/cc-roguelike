...
import { shuffleArray, tileAlign } from '../../utils/dungeon';
...
export function generateColosseum(floor: number, seed: number): ArenaData {
  const terrain = colosseumGenerator.generate(floor, seed);
  // Extract exitDoorId from envObjects (first door object)
  const doorObj = terrain.envObjects.find(o => o.type === 'door');
  return {
    room: terrain.rooms![0] ?? { x: 0, y: 0, width: 0, height: 0, type: 'arena' },
    collisionGrid: terrain.collisionGrid,
    envObjects: terrain.envObjects,
    spawnPoint: terrain.spawnPoint,
    exitPoint: terrain.exitPoint,
    exitDoorId: doorObj?.id ?? '',
  };
}

export const colosseumGenerator: TerrainGenerator = {
  type: 'colosseum',
...
registerTerrain(colosseumGenerator);
