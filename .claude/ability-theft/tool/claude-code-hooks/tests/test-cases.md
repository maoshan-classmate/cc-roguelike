...
**识别到的边界场景（来自资源原文）：**
1. Windows CRLF 兼容性 — 来源：pua-loop-hook.sh（tr -d '\r' 处理）
2. python3/jq 缺失时的降级处理 — 来源：failure-detector.sh, stop-feedback.sh
3. SessionStart additionalContext vs 纯文本 stdout 的优先级差异 — 来源：session-restore.sh
4. Stop hook 的 decision:block 阻止退出机制 — 来源：pua-loop-hook.sh
5. 会话隔离（session_id 绑定）— 来源：failure-detector.sh, pua-loop-hook.sh
6. Compaction 后状态恢复的 2 小时过期机制 — 来源：session-restore.sh

**核心能力点（来自资源原文）：**
1. hooks.json 注册格式和 matcher 规则 — 来源：hooks/hooks.json
2. additionalContext JSON 注入机制 — 来源：hooks/session-restore.sh
3. 风味系统（flavor-helper.sh）的共享 source 模式 — 来源：hooks/flavor-helper.sh
4. 失败检测 + 计数器 + 压力升级闭环 — 来源：hooks/failure-detector.sh
5. Loop 控制（Stop hook block + 状态文件 + transcript 读取）— 来源：hooks/pua-loop-hook.sh
...
### TC-05：风味系统共享 source 模式
**目标任务：** 创建 flavor-helper.sh，多个 hook 脚本共享，通过 config.json 配置当前风味
**预期入口：** mechanisms/flavor-routing.md → scripts/flavor-helper-template.sh
**成功标准：** get_flavor() 正确读取配置、设置环境变量、各 hook 能使用风味变量
**类型：** 核心能力
**结果：** 待验证
...
**成功标准：** compaction 前状态写入 builder-journal.md，compaction 后 SessionStart hook 正确恢复
**类型：** 边界场景
**结果：** 待验证
