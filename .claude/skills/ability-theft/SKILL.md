...
**⚠️ 强制阻塞规则：步骤 B 必须用 AskUserQuestion 向用户确认命名和分类，收到用户回复后才能进入步骤 C。禁止将命名确认合并到其他问题中，禁止跳过此步直接进入窃取等级选择。**
...
**步骤 B 完成条件（强制检查）：**
```
□ 已用 AskUserQuestion 向用户展示建议命名和分类
□ 已收到用户对命名的明确确认（使用建议名 / 自定义名）
□ 已收到用户对分类的明确确认（使用推荐分类 / 自定义分类）
□ 以上两项都有回复后，才能进入步骤 C
```
