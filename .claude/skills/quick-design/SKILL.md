...
**Output:** `docs/gdd/quick-specs/[name]-[date].md`
...
- Search `docs/gdd/` for the GDD most relevant to this change. Read the
  sections that this change would affect.
- Check whether `docs/gdd/systems-index.md` exists. If it does, read it to
...
- Check `docs/gdd/quick-specs/` for any prior quick specs that touched this
...
**Type**: Tuning
**System**: [System name]
**GDD Reference**: `docs/gdd/[filename].md` — Tuning Knobs section
**Date**: [today]
...
**Type**: [Tweak / Addition]
**System**: [System name]
**GDD Reference**: `docs/gdd/[filename].md`
**Date**: [today]
...
Current GDD says (quoting `docs/gdd/[filename].md`, [section]):
...
`docs/gdd/quick-specs/[kebab-case-title]-[YYYY-MM-DD].md`?"
...
If yes, create the `docs/gdd/quick-specs/` directory if it does not exist, then
write the file.
...
`docs/gdd/[filename].md` — specifically the [section name] section?"
...
```
Quick Design Spec written to: docs/gdd/quick-specs/[filename].md
