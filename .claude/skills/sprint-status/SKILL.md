...
- If an argument is given (e.g., `/sprint-status 3`), search
  `docs/todo/` for a file matching `sprint-03.md`, `sprint-3.md`,
  or similar. Report which file was found.
- If no argument is given, find the most recently modified file in
  `docs/todo/` and treat it as the current sprint.
- If `docs/todo/` does not exist or is empty, report: "No sprint
...
**First: check for `.claude/session-state/sprint-status.yaml`.**
...
Optionally (fast check only — do not do a deep scan): grep `src/pages/` for a
