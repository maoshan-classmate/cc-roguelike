...
```bash
git log --name-only --pretty=format: -- docs/gdd/ | grep "\.md$" | sort -u
```
Limit to GDDs modified since the most recent `docs/gdd/gdd-cross-review-*.md`
...
```
Grep pattern="[entity_name]" glob="docs/gdd/*.md" output_mode="content" -C 3
```
...
```
Read path="docs/gdd/[conflicting_gdd].md"
```
