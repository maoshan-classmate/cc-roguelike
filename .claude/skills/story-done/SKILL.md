...
2. Else read `.claude/session-state/review-mode.txt` -> use that value
3. Else -> default to `lean`

**If a file path is provided** (e.g., `/story-done docs/todo/core/story-damage-calculator.md`):
read that file directly.

**If no argument is provided:**

1. Check `.claude/session-state/active.md` for the currently active story.
2. If not found there, read the most recent file in `docs/todo/` and
...
| **Visual/Feel** | Screenshot + sign-off in `docs/bugs/evidence/` | ADVISORY |
| **UI** | Manual walkthrough doc OR interaction test in `docs/bugs/evidence/` | ADVISORY |
| **Config/Data** | Smoke check pass report in `docs/bugs/smoke-*.md` | ADVISORY |
...
`tests/integration/[system]/` broadly, then check `.claude/session-state/` for a
playtest record referencing this story.
If none found: flag as **BLOCKING** (same rule as Logic).

**For Visual/Feel and UI stories**: glob `docs/bugs/evidence/` for a file
referencing this story. If none: flag as **ADVISORY** —
"No manual test evidence found. Create `docs/bugs/evidence/[story-slug]-evidence.md`
using the test-evidence template and obtain sign-off before final closure."

**For Config/Data stories**: check for any `docs/bugs/smoke-*.md` file.
...
4. **Update `.claude/session-state/sprint-status.yaml`** (if it exists):
...
`.claude/session-state/active.md`:
...
1. Read the current sprint plan from `docs/todo/`.
