...
2. Else read `.claude/review-mode.txt` → use that value
...
1. Check if `docs/gdd/systems-index.md` exists.
...
3. If no systems index exists, fail with:
   ...
   > Or to fill gaps in an existing GDD: `/design-system retrofit docs/gdd/[system-name].md`
...
existing `.md` file in `docs/gdd/`, enter **retrofit mode**:
...
- **Game concept**: Read `docs/gdd/game-concept.md` — fail if missing:
  > "No game concept found. Run `/brainstorm` first."
- **Systems index**: Read `docs/gdd/systems-index.md` — fail if missing:
...
- **Game pillars**: Read `docs/gdd/game-pillars.md` if it exists
- **Existing GDD**: Read `docs/gdd/[system-name].md` if it exists (resume, don't
  restart from scratch)
- **Related GDDs**: Glob `docs/gdd/*.md` and read any that are thematically related
...
Ask: "May I create the skeleton file at `docs/gdd/[system-name].md`?"

After writing, update `.claude/session-state/active.md`:
...
After writing each section, update `.claude/session-state/active.md` with the
...
Pass: completed GDD file path, game pillars (from `docs/gdd/game-concept.md` or `docs/gdd/game-pillars.md`), MDA aesthetics target.
...
> `/design-review docs/gdd/[system-name].md`
>
...
- Update the target system's row:
  ...
  - Design Doc: link to `docs/gdd/[system-name].md`
- Update the Progress Tracker counts

Ask: "May I update the systems index at `docs/gdd/systems-index.md`?"

### 5d: Update Session State

Update `.claude/session-state/active.md` with:
...
1. Read `.claude/session-state/active.md` — it records the current system and
   which sections are complete
2. Read `docs/gdd/[system-name].md` — sections with real content are done;
...
> sections are written to `docs/gdd/[system-name].md`. When you're ready to continue,
...
- Run `/design-review docs/gdd/[system-name].md` in a **fresh session** to validate the completed GDD independently
