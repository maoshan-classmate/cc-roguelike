...
1. Detect the current sprint or feature scope from the argument:
   - If argument is a sprint identifier (e.g., `sprint-03`): read all story files in `docs/todo/[sprint]/`
   - If argument is `feature: [system-name]`: glob story files tagged for that system
   - If no argument: read `.claude/session-state/active.md` and `.claude/session-state/sprint-status.yaml` (if present) to infer the active sprint

2. Read `.claude/session-state/stage.txt` to confirm the current project phase.
...
Ask: "May I write the QA plan to `docs/bugs/qa-plan-[sprint]-[date].md`?"
...
After each FAIL result: use `AskUserQuestion` to collect the failure description, then spawn `qa-tester` via Task to write a formal bug report in `docs/bugs/`.
...
Ask: "May I write this QA sign-off report to `docs/bugs/qa-signoff-[sprint]-[date].md`?"
