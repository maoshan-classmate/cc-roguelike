...
**Output:** `docs/bugs/smoke-[date].md`
...
3. **Smoke test list**: check whether `docs/bugs/smoke-tests.md` or
...
4. **QA plan check**: glob `docs/bugs/qa-plan-*.md` and take the most
...
Report findings before proceeding: "Environment: React 18 + Node.js. Test directory:
...
Attempt to run the test suite via Bash.

**For this project (React + Node.js):**
```bash
npx tsc --noEmit 2>&1
```
Check for TypeScript compilation errors. Also check for any test runner:
```bash
npm test 2>&1
```

**If the test runner is not available in this environment**, report clearly:

"Automated tests could not be executed — test runner not found.
...
2. The current sprint plan from `docs/todo/` (most recently modified
   file)
...
1. Extract the system slug from the story's file path
   (e.g., `docs/todo/combat/story-001.md` -> `combat`)
...
2. `docs/bugs/smoke-tests.md` (if it exists)
...
**Engine**: React 18 + Node.js + Socket.io
...
"May I write this smoke check report to `docs/bugs/smoke-[date].md`?"
...
QA hand-off: share `docs/bugs/qa-plan-[sprint].md` with the qa-tester
