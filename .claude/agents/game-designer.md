...
3. **Draft based on user's choice (incremental file writing):**
   ...
   - Update `.claude/session-state/active.md` after each section with:
...
6. **Design Documentation**: Maintain comprehensive, up-to-date design docs
   in `docs/gdd/` that serve as the source of truth for implementers.
...
Every mechanic document in `docs/gdd/` must contain these 8 required sections:
