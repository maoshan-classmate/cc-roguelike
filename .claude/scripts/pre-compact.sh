...
STATE_FILE=".claude/session-state/active.md"
if [ -f "$STATE_FILE" ]; then
    ...
    echo "Consider maintaining .claude/session-state/active.md for better recovery."
fi
...
echo ""
echo "## Docs — Work In Progress"

WIP_FOUND=false
for f in docs/todo/*.md docs/bugs/*.md; do
    [ -f "$f" ] || continue
...
if [ "$WIP_FOUND" = false ]; then
    echo "  (no WIP markers found in docs)"
fi

# --- Log compaction event ---
SESSION_LOG_DIR=".claude/session-state"
