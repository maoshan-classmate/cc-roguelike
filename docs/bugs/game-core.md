...
## 8. 怪物卡住无法移动 (2026-05-09)
- **优先级**: P0
- **发现时间**: 2026-05-09
- **问题**: 怪物在移动过程中会卡在墙壁角落或互相卡住，永久无法移动
- **期望行为**: 怪物碰到障碍物应绕行，卡住时有恢复机制
- **根因分析**:
  - **根因 #1**: `CollisionGrid.ts:63` — `dist > 0` 守卫跳过 `dist === 0` 的情况。两个敌人在完全相同坐标时分离力为零，永久粘连。同位置的敌人都朝玩家移动，方向一致，永远无法分离
  - **根因 #2**: `EnemyAI.ts:118-138` — 敌人碰到凹形墙角/走廊入口时，所有 8 个逃逸偏移都可能落在墙内，位置永远不更新。AI 每帧重新计算相同路径，无限循环
  - **根因 #3**: 敌人 AI 状态机只有 `idle/chase/attack` 三种状态，无 "stuck" 检测和恢复机制。卡住后永久停留在 `chase` 状态
  - **加剧因素**: `GameRoom.ts:505-509` 先运行 AI 移动再运行碰撞分离，允许多帧重叠累积
- **修复方案**:
  1. `CollisionGrid.ts:63` 处理 `dist === 0`：施加随机方向分离力
  2. `EnemyAI.ts` 添加卡住检测：N 帧位置不变 → 随机漫游恢复
  3. `GameRoom.ts` 调整更新顺序：先分离再移动（或两步都做）
  4. 敌人逃逸偏移失败时，增加完整 tile 步长搜索
- **涉及文件**:
  - `server/game/collision/CollisionGrid.ts` — separateEnemies dist=0 问题
  - `server/game/enemy/EnemyAI.ts` — 逃逸偏移 + 卡住检测
  - `server/game/GameRoom.ts` — 更新顺序
- **涉及系统**: 碰撞检测、敌人 AI
- **修复方案**:
  1. `CollisionGrid.ts` separateEnemies: dist=0 时施加随机方向分离力
  2. `EnemyAI.ts` 添加卡住检测: stuckTimers Map 追踪位置变化，30帧不动触发随机漫游
  3. 常规敌人和 Boss 逃逸偏移后增加 8 方向 tile 步长兜底搜索
  4. 新增 TILE_SIZE import
- **修复提交**: 2026-05-09
- **状态**: ✅ 已修复
