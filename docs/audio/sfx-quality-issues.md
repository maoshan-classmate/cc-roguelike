...
**采用方案 B**：CC0 外部素材替换关键音效。方案 C（Tone.js 后处理）因 Node.js 缺少 `OfflineAudioContext` 跳过。
...
- `scripts/postprocess-sfx.js`：新建（Tone.js 后处理），因 Node.js 限制暂不可用

### 方案 C 状态

Tone.js 后处理脚本已创建但无法在 Node.js 运行（缺少 `OfflineAudioContext`）。需在浏览器构建时或使用 polyfill 重新启用。
