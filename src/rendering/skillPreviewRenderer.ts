...
const PREVIEW_RENDERERS: Record<string, (pc: PreviewCtx) => void> = {
  line(pc) {
    const { ctx, preview, def, breathe } = pc
    // Dash direction line
  ...
  shockwave(pc) {
    const { ctx, preview, def, breathe, spin } = pc
  ...
  target_fire(pc) {
    const { ctx, preview, def, breathe, spin, elapsed } = pc
  ...
  reticle(pc) {
    ...
    ctx.moveTo(preview.x, preview.y + cSize); ctx.lineTo(preview.x, preview.y + cSize)
    ctx.stroke()
